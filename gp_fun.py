...
class GP(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, dim=None, kernel='Matern'):
        ...
        # Use excact GP instead of KISS-GP
        if kernel == 'Matern':
            if train_x is None:
            ...
            if train_x is None:
                assert dim is not None
                self.covar_module =gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel(ard_num_dims=dim, lengthscale_constraint=lengthscale_constraint))
            else:
                self.covar_module =gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel(ard_num_dims=train_x.shape[1], lengthscale_constraint=lengthscale_constraint)) 
        elif kernel == 'Periodic':
            if train_x is None:
                assert dim is not None
                self.covar_module =gpytorch.kernels.ScaleKernel(gpytorch.kernels.PeriodicKernel(lengthscale_constraint=lengthscale_constraint))
            else:
                self.covar_module =gpytorch.kernels.ScaleKernel(gpytorch.kernels.PeriodicKernel(lengthscale_constraint=lengthscale_constraint)) 
        elif kernel == 'RQ':
            if train_x is None:
                assert dim is not None
                self.covar_module =gpytorch.kernels.ScaleKernel(gpytorch.kernels.RQKernel(ard_num_dims=dim, lengthscale_constraint=lengthscale_constraint))
            else:
                self.covar_module =gpytorch.kernels.ScaleKernel(gpytorch.kernels.RQKernel(ard_num_dims=train_x.shape[1], lengthscale_constraint=lengthscale_constraint)) 
