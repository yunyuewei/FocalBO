...
from gp_model import train_gp, train_svgp, train_fvgp, train_spgp, train_vecchia
...
if __name__ == '__main__':
    ...
    if 'Morphology' in args.task or 'Super' in args.task:
    ...
    with gpytorch.settings.max_cholesky_size(float('inf')):
        for rep in range(n_repeat):
            ...
            train_Y = initial_Y[:n_initial_samples].clone().reshape(-1, 1)
                # print(train_Y.shape)
                # raise NotImplementedError
            ...
            while len(train_X) < n_evals:
                ...
                print(train_X.shape, train_Y.shape, norm_Y.shape)
                    ...
                    if gp_model == 'fvgp':
                        try:
                            ...
                            if algo == 'turbo':
                                ...
                                lb = tr_lb.expand(len(train_X), len(tr_lb.flatten()))
                                ub = tr_ub.expand(len(train_X), len(tr_ub.flatten()))
                        ...
                        model, likelihood = train_spgp(train_X, train_Y, num_epochs, dim)
                        tr = cand_tr =  torch.cat((torch.zeros(dim).reshape(1, -1), torch.ones(dim).reshape(1, -1))).to(dtype=dtype, device=device)
                ...
                if gp_model == 'fvgp' or args.use_depth ==1 and len(train_X) >= induce_size: # sparse to dense optimization for fvgp and vanilla BO
                    ...
                    if acqf == 'ucb':
                        ...
                        tr_lb = torch.clamp(x_center - state.length / 2.0, 0, 1).reshape(1, -1)
                        tr_ub = torch.clamp(x_center + state.length / 2.0, 0, 1).reshape(1, -1)
                        bound = torch.cat((tr_lb, tr_ub))
                        print(bound)
                        # raise NotImplementedError
                        new_X = expected_improvement(model, likelihood, best_f=norm_Y.max(), batch_size=n_samples_per_step, bound=bound, device=device)
                    elif acqf == 'qei':
                        print('use qei')
                        ...
                        print('use ts')
                        if algo == 'turbo':
                            print("base_kernel", model.covar_module.base_kernel)
